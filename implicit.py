...
class HarmonicEmbedding(torch.nn.Module):
    def __init__(
        ...
        self.register_buffer("_frequencies", omega0 *
                             frequencies, persistent=False)
...
class NeuralRadianceField(torch.nn.Module):
    def __init__(
        ...
        #self.linear0 = torch.nn.Linear(3, 256)
        #self.linear1 = torch.nn.Linear(39, 256)
    ...
    def forward(self, ray_bundle):
        #print("sample points shape: {}".format(ray_bundle.sample_points.shape))
        # print("sample points shape: {}".format(
        #    ray_bundle.sample_points.view(-1, 3).shape))
        #out = self.harmonic_embedding_xyz(ray_bundle.sample_points)
        #out = self.linear1(ray_bundle.origins)
        ...
        #out = self.linear1(self.relu(out))
        #out = self.relu(out)
        #out = self.sigmoid(out)
        #print("density shape: {}".format(out["density"].shape))
        #print("feature shape: {}".format(out["feature"].shape))
        # out = {
        #    'density': -torch.log(1.0 - density) / deltas,
        #    'feature': base_color * self.feature * density.new_ones(sample_points.shape[0], 1)
        # }
