    ...
    def reshape(self, *args):
        return RayBundle(
            ...
            self.sample_lengths.reshape(
                *args, self.sample_lengths.shape[-2], 1),
        )
...
def get_random_pixels_from_image(n_pixels, image_size, camera):
    ...
    xy_grid_sub = xy_grid_sub.index_select(
        0, torch.randperm(xy_grid_sub.shape[0]))
...
def get_rays_from_pixels(xy_grid, image_size, camera):
    ...
    ndc_points = torch.cat(
        (xy_grid, torch.ones(xy_grid.shape[0], 1)), 1).cuda()
    ...
    #cam_mat = camera.get_camera_center()
    #rays_o = cam_mat - xyz_world

    # TODO (1.3): Get normalized ray directions
    #rays_d = rays_o / torch.linalg.vector_norm(rays_o, dim=1).view(-1, 1)
